...
- 不重新開發完整新系統。
- 沿用目前靜態網站、每日教材、SRS、任務情境與音檔產線；`The Blue Receipt` 保留為已封存的歷史內容。
- 將目前系統從「每日教材產生器」升級成「個人英文訓練作業系統」，每日核心改為可驗收的 real-life mission。
...
- [x] 顯示累計天數、總單字、已掌握、今日到期、逾期、目前訓練模組。
- [x] 首頁加入今日訓練路線。
...
- [x] 新增任務型頁面需驗證 Mission、Role-play 與 Context Recall 最低題數；歷史小說頁保持相容。
- [ ] 若使用者明確要求小說工作，才檢查連載模式是否更新 `CONTINUITY_LOG.md`。
...
- 顯示今日訓練路線。
- 使用現有 JSON 即時計算學習狀態。
- 新模式每日頁增加 Mission 與 Role-play 區塊；歷史 daily 頁不做大規模重寫。
