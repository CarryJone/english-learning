...
- [x] Day 77–113 已完成三十七篇任務型教材；最新為 `daily/2026-09-07/` 的 `Storing Bags in a Station Locker Before a Day Trip`。
...
## 驗證狀態
- [x] Day 113 通過 `python3 scripts/validate_daily.py 2026-09-07`：84 checks、0 warnings、0 errors；主音檔 129.63 秒，35 句 / 285 字、每句最多 10 字，36 個 MP3 均存在且可讀。
- [x] Day 113 包含 3 個新字、60 題到期 Active Recall、10 題 Context Recall、4 組 Speaking Bridge 與 10 回合 Role-play；375 / 390px 手機寬度無橫向溢位、console 0 errors、首頁最新一筆為 Day 113。
- [x] Day 112（2026-09-03）先前同樣通過 84 checks；主音檔 123.72 秒、34 句 / 280 字。
...
## 下一步
- 先看 Day 112、113 的理解、停頓與視覺重音回饋；若聽力仍低於約六成，優先再減少資訊密度與陌生搭配，不直接降低自然 A2 語速。
- 到期單字量已成長到單日 60 題，下一輪評估 Active Recall 是否需要分批、分頁或優先序 UI。
