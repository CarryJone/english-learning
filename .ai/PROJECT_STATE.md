...
- [x] Day 77–97 已完成二十一篇正式任務型教材；最新為 `daily/2026-08-13/` 的 `Buying Sunscreen for a Beach Day`。
...
- [x] Day 97 目標語塊 `How often should I reapply it?`、`Is this okay for sensitive skin?`、`Do you have a smaller bottle?` 已貫穿 Article、Key Phrases、Role-play 的「你」回合與 Context Recall。
...
## 驗證狀態
- [x] Day 96 通過 `python3 scripts/validate_daily.py 2026-08-12`：84 checks、0 warnings、0 errors；主音檔 134.0 秒，36 句 / 251 字、每句最多 10 字，37 個 MP3 均存在且可讀。
- [x] Day 96 包含 3 個新字、16 題到期 Active Recall、10 題 Context Recall、4 組 Speaking Bridge 與 8 回合 Role-play；390×844 手機版無橫向溢位，0.75× 實際生效，修正彈窗初始化後 console 0 errors。
- [x] Day 97 通過 `python3 scripts/validate_daily.py 2026-08-13`：84 checks、0 warnings、0 errors；主音檔 123.1 秒，36 句 / 260 字、每句最多 10 字，37 個 MP3 均存在且可讀。
...
## 下一步
- 下一篇先看 Day 97 的理解回饋；若聽力仍低於約六成，優先再減少資訊密度與陌生搭配，不直接降低自然 A2 語速。
