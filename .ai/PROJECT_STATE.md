...
- [x] Day 77–84 已完成八篇正式任務型教材；最新為 `daily/2026-07-27/` 的 `Fixing a Broken Rental Bike`。
- [x] Day 81–84 正在進行 7 篇語塊深化試行（4/7）；Day 84 目標語塊為 `report a broken lock`、`choose a replacement bike`、`Could you say that again?`。
...
## 驗證狀態
- [x] Day 84 通過 `python3 scripts/validate_daily.py 2026-07-27`：79 checks、0 warnings、0 errors。
- [x] Day 84 包含 3 個新字、44 題到期 Active Recall、10 題 Context Recall、4 組 Speaking Bridge、9 回合 Role-play 與 15 個非空音檔。
- [x] Day 84 已完成頁面與 Day 83 的瀏覽器版面比對；桌面寬度、卡片寬度、水平溢位均一致，單句單次／循環播放均已實測。
- [x] Day 84 的 3 個目標語塊已分布在 Article、Key Phrases、Role-play 的 You 回合與 Context Recall，並以機場情境做跨情境提取。

## 目前 Blocker
- 無。

## 下一步
- 產出 Day 85 前先同步最新 SRS，進行語塊深化試行第 5/7 篇，並依本週能力平衡選擇下一個旅行開口或英文資訊判讀任務。
