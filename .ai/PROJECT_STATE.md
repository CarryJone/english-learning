...
- [x] Day 77–122 已完成四十六篇任務型教材；最新為 `daily/2026-09-18/` 的 `Checking If Our Boat Tour Is Still On`（M4，資訊判讀＋開口）。自然口語規則、四聲線與 Survival Lines 自 Day 117 起為常態流程。
...
- [x] 模組近期採逐日輪替：Day 118 M4 → 119 M2 → 120 M3 → 121 M1 → 122 M4；下一篇依序可排 M2 `Hotels, Food & Shopping`。

## 驗證狀態
- [x] Day 122（2026-09-18）通過 **97 checks** / 0 warnings / 0 errors；主音檔 130.47 秒、36 句 / 263 字、37 個 MP3 均可讀。
- [x] Day 122 實測：對話 **89%**、含縮寫句 **36%**、最長旁白連續 **1**；四聲線（staff＝遊船客服、companion＝同行朋友）；375 與 390px 皆無橫向溢位、console 0 errors、42 個音檔（含核心句）回 200。今日到期舊單字 10 個（09-17 的 14 題只錯 `eligible`）。
- [x] Survival Lines：09-15 與 09-17 有同步（09-17：could-you-help / didnt-catch / do-you-speak-english 皆 remembered），09-16 未同步。Day 122 換成 how-much / how-spell / id-like-this-one / pay-by-card / say-again。
...
- 首頁學習記錄缺 `daily/2026-07-28/`（Day 85 `Reporting a Damaged Suitcase`）：121 筆連結對應 122 個資料夾。既有問題，尚未處理。
- 每日頁產生器（build_html / tts / update_data / core_block）只存在 session scratchpad，不在 repo。**2026-09-17 實際發生：scratchpad 被系統清空，腳本遺失**，當天依對話紀錄重建並與 Day 120 頁面比對 CSS、inline JS、區塊標題完全一致；Day 122 沿用重建版。
...
- 到期量波動大（Day 118 有 43 題、Day 119 只有 3 題、Day 120 有 19 題、Day 121 有 14 題、Day 122 有 10 題）；若再出現單日 50 題以上，再評估 Active Recall 是否需要分批或分頁 UI。
...
## 活躍工作區
- `daily/2026-09-18/`、`index.html`、`profile.json`、`ability_map.json`
