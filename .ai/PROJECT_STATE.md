...
## 驗證狀態
- [x] Day 116 通過 `python3 scripts/validate_daily.py 2026-09-10`：84 checks、0 warnings、0 errors；主音檔 127.95 秒，35 句 / 281 字、每句最多 10 字，36 個 MP3 均存在且可讀。
- [x] Day 116 包含 3 個新字、12 題到期 Active Recall、10 題 Context Recall、4 組 Speaking Bridge 與 10 回合 Role-play；375px 手機寬度無橫向溢位、console 0 errors、首頁最新一筆為 Day 116。
- [x] Day 115（2026-09-09）先前同樣通過 84 checks；主音檔 124.92 秒、35 句 / 273 字、12 題到期 Active Recall。
...
## 下一步
- 先看 Day 113–116 的理解、停頓與視覺重音回饋；若聽力仍低於約六成，優先再減少資訊密度與陌生搭配，不直接降低自然 A2 語速。
