...
- [x] Day 77–120 已完成四十四篇任務型教材；最新為 `daily/2026-09-16/` 的 `Fixing a Missing Hotel Booking at Check-in`（M3，資訊判讀＋問題處理）。自然口語規則、四聲線與 Survival Lines 自 Day 117 起為常態流程。
...
- [x] 模組近期採逐日輪替：Day 116 M3 → 117 M1 → 118 M4 → 119 M2 → 120 M3；下一篇依序可排 M1 `Transport & Getting Around`。

## 驗證狀態
- [x] Day 120（2026-09-16）通過 **97 checks** / 0 warnings / 0 errors；主音檔 136.06 秒、36 句 / 254 字、37 個 MP3 均可讀。
- [x] Day 120 實測：對話 **89%**、含縮寫句 **31%**、最長旁白連續 **1**；四聲線（staff＝飯店櫃檯、companion＝同行朋友）；375 與 390px 皆無橫向溢位、console 0 errors、42 個音檔（含核心句）回 200。今日到期舊單字 19 個。
- [x] Survival Lines：學習者 2026-09-15 首次完整作答並同步（4 句 remembered、`walking-distance` forgot）；Day 120 依到期與最久未用換成 can-i-have（變化 2）/ check-please / what-recommend / where-restroom / wheres-nearest。
...
- 首頁學習記錄缺 `daily/2026-07-28/`（Day 85 `Reporting a Damaged Suitcase`）：119 筆連結對應 120 個資料夾。既有問題，尚未處理。
...
- 到期量波動大（Day 118 有 43 題、Day 119 只有 3 題、Day 120 有 19 題）；若再出現單日 50 題以上，再評估 Active Recall 是否需要分批或分頁 UI。
...
## 活躍工作區
- `daily/2026-09-16/`、`index.html`、`profile.json`、`ability_map.json`
