...
- [x] Day 77–86 已完成十篇正式任務型教材；最新為 `daily/2026-07-29/` 的 `Changing a Train Ticket After a Delay`。
- [x] Day 81–86 正在進行 7 篇語塊深化試行（6/7）；Day 86 目標語塊為 `look up the change rule`、`Is this ticket still valid?`、`Could you say that again?`。
...
## 驗證狀態
- [x] Day 86 通過 `python3 scripts/validate_daily.py 2026-07-29`：79 checks、0 warnings、0 errors。
- [x] Day 86 包含 3 個新字、9 題到期 Active Recall、10 題 Context Recall、4 組 Speaking Bridge、8 回合 Role-play 與 14 個非空音檔。
- [x] Day 86 的 3 個目標語塊已分布在 Article、Key Phrases、Role-play 的 You 回合與 Context Recall，並以公車一日票做跨情境提取。

## 目前 Blocker
- 無。

## 下一步
- 產出 Day 87 前先同步最新 SRS，完成語塊深化試行第 7/7 篇，並依本週能力平衡選擇下一個旅行開口或英文資訊判讀任務。
