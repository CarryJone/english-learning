...
- [x] Day 77–112 已完成三十六篇正式任務型教材；最新為 `daily/2026-09-03/` 的 `Checking a City Bike Before a Museum Ride`。
...
- [x] Day 112 通過 `python3 scripts/validate_daily.py 2026-09-03`：84 checks、0 warnings、0 errors；主音檔 123.72 秒，34 句 / 280 字，35 個 MP3 均存在且可讀。
- [x] Day 112 包含 3 個新字、18 題到期 Active Recall、10 題 Context Recall、4 組 Speaking Bridge 與 10 回合 Role-play；任務聚焦讀懂城市單車租借規則、檢查車況與處理替代車輛。
- [x] Day 112 首次在當日頁面試行視覺句子重音：重音可切換顯示 / 隱藏，逐句播放時標亮目前句子；今日新字顯示 `PED-al`、`BRAKE`、`HEL-met`。音檔維持自然語速，未加入人工重音。

## 目前 Blocker
- 無。

## 下一步
- 下一篇先看 Day 112 的理解、停頓與視覺重音回饋；若聽力仍低於約六成，優先再減少資訊密度與陌生搭配，不直接降低自然 A2 語速。
