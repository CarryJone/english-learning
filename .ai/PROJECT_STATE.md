...
- [x] Day 77–115 已完成三十九篇任務型教材；最新為 `daily/2026-09-09/` 的 `Buying a Warm Jacket After the Weather Turns Cold`。
...
## 驗證狀態
- [x] Day 115 通過 `python3 scripts/validate_daily.py 2026-09-09`：84 checks、0 warnings、0 errors；主音檔 124.92 秒，35 句 / 273 字、每句最多 11 字，36 個 MP3 均存在且可讀。
- [x] Day 115 包含 3 個新字、12 題到期 Active Recall、10 題 Context Recall、4 組 Speaking Bridge 與 10 回合 Role-play；375px 手機寬度無橫向溢位、console 0 errors、首頁最新一筆為 Day 115。
- [x] Day 114（2026-09-08）先前同樣通過 84 checks；主音檔 132.34 秒、35 句 / 274 字、3 題到期 Active Recall。
...
## 下一步
- 先看 Day 113–115 的理解、停頓與視覺重音回饋；若聽力仍低於約六成，優先再減少資訊密度與陌生搭配，不直接降低自然 A2 語速。
- 到期量波動大（Day 113 有 60 題、Day 114 只有 3 題、Day 115 有 12 題）；若之後再出現單日 50 題以上，再評估 Active Recall 是否需要分批或分頁 UI。
- Speaking Bridge 候選池會週期性變小。Day 115 起在同為最低 reviewCount 的候選中，優先選尚未入選過的字（本次為 `insert` / `medium` / `space`），仍符合 SKILL 的 reviewCount 優先規則；若要改成硬性「同一字不得連續入選」，需先更新 SKILL。
