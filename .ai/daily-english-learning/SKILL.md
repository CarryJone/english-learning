...
- 同一個意思有「標準單動詞」與「日常口語片語」兩種說法時，**一律選片語**：

| 不要用 | 改用 |
|---|---|
| `finish work` | `get off work` |
| `bring it to my hotel` | `drop it off at my hotel` |
| `collect` / `retrieve` | `pick it up` |
| `return it to the shop` | `take it back` |
| `board the bus` | `get on the bus` |
| `depart` | `leave` / `head out` |

- 理由：這些片語是母語人士日常對話的預設說法，而且**片語比單字更容易跨情境轉移**（`drop off` 可用於行李、包裹、小孩、租車）。
- 但**不要為了口語而選少見俚語**。片語必須是 A2 學習者查得到、聽得懂、明天就用得上的高頻組合；不確定的搭配要用可靠學習字典或語料例句確認，不能只因為「聽起來比較口語」就採用。
- 選到的片語若符合 3c 的類型需求，優先納入當天的 Key Phrases。
...
| `companion` | 本篇第二個說話者 | `en-US-BrianNeural`（男） | 同行朋友，**或**第二個場景的另一位服務人員（例如先跟司機通話、再到櫃檯領件） | 否 |

**硬規則**

- **同一篇教材內，一個 role 只能代表一個人。** 不可用同一個 `staff` 同時演「朋友」和「司機」——那會讓兩個人共用一個聲音，學習者分不出誰在講話。
- 一篇最多 4 個聲線。
- 若某篇任務出現**第二個服務方**（例如先跟司機通話、再到失物招領櫃檯領件），第二個人必須用 `companion`，**不可讓兩個人共用 `staff`**。旁白仍要在切換前點名（例如 `The next morning I go to the office.`），讓學習者知道換場景、換人了。
- `companion` 有兩種用途，一篇只能擇一：
  - **同行者**：把「旁白唸資訊」轉成對話（見 3a.1 規則 B）。研究、查資料、討論選項的場景優先讓同行者出場。
  - **第二個服務人員**：支援跨場景任務（電話 → 現場、櫃檯 → 另一個窗口）。
- 若某篇同時需要同行朋友**和**第二個服務方（等於 5 個說話者），超出 4 聲線上限，必須重構情境把說話者壓到 4 個以內，不可新增第五個角色。
- 不需要第二個說話者時就不要硬加角色。
...
- **片語選擇原則**：
  ...
  - 依 3a.1 規則 D，若文章用到 `get off work`、`drop off`、`pick up`、`take back` 這類取代標準單動詞的口語片語，優先把它們列進 Key Phrases
...
<div class="container">
  ...
  <div class="player-card">
    <div class="player-label">🎧 2-Minute Listening Practice</div>
...
- `[ROLEPLAY_HTML]`：填入至少 4 個回合的 Role-play，每回合使用 `<div class="roleplay-turn"><div class="roleplay-speaker">店員 / 你</div><div class="roleplay-line">英文句子</div></div>`；第三或第四回合必須加入資訊改變、聽不懂、澄清或替代方案分支，並讓「你」至少有 2 次需要自己產出英文。「你」的回合合計必須自然使用今天的 2–3 個目標語塊，不可只讓店員說出。
...
async def gen(text, speaker, path):
    c = edge_tts.Communicate(text, voice=VOICES[speaker], rate=RATE)
    await c.save(path)

async def main():
    paths = []
    for i, item in enumerate(SENTENCES, 1):
        path = f"{FOLDER}/s{i:02d}.mp3"
    ...
    command.extend([
        "-filter_complex", f"{inputs}concat=n={len(paths)}:v=0:a=1[outa]",
        "-map", "[outa]", "-ar", "24000", "-ac", "1", "-b:a", "48k",
        f"{FOLDER}/article.mp3",
    ])
